...
// BvmContract contractInfo of BVM
type BvmContract struct {
	Address      types.Address `json:"address"`      //合约地址
	Deployer     types.Address `json:"deployer"`     //合约部署者账户地址
	BvmAbi       string        `json:"bvmAbi"`       //合约Abi文件
	Methods      []BvmMethod   `json:"methods"`      //合约对外提供接口的方法列表
	Events       []string      `json:"events"`       //合约提供的事件接口列表
	Token        types.Address `json:"token"`        //合约绑定的代币地址
	ChainVersion int64         `json:"chainVersion"` //链版本
}

// Method method information of BVM
type BvmMethod struct {
	MethodID  string `json:"methodId"`  //方法ID
	ProtoType string `json:"prototype"` //方法原型
}
