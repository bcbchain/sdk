...
import (
	...
	"github.com/AeReach/sdk/sdk/ibc"
...
type IAccount interface {
	...
	BalanceOfNameBRC30(orgName, name string) bn.Number                                                           //根据组织名称和代币名称获取BRC30代币余额(cong)
	BalanceOfSymbol(symbol string) bn.Number                                                                     //根据符号获取代币或基础通证的余额（cong）
	BalanceOfSymbolBRC30(orgName, symbol string) bn.Number                                                       //根据组织名称和代币符号获取BRC30代币余额(cong)
	...
	TransferByNameBRC30(orgName string, name string, to types.Address, value bn.Number)                          //根据组织名称及代币名称向指定账户进行指定的BRC30代币的转账
	TransferByNameWithNote(name string, to types.Address, value bn.Number, note string)                          //根据名称执行代币或基础通证的转账，收据中包含 note 信息（cong）
	TransferByNameWithNoteBRC30(orgName string, name string, to types.Address, value bn.Number, note string)     //根据组织名称及代币名称向指定账户进行指定的BRC30代币的转账,收据中包含note信息
	TransferBySymbol(symbol string, to types.Address, value bn.Number)                                           //根据符号执行代币或基础通证的转账（cong）
	TransferBySymbolBRC30(orgName string, symbol string, to types.Address, value bn.Number)                      //根据组织名称及代币符号向指定账户进行指定的BRC30代币的转账
	TransferBySymbolWithNote(symbol string, to types.Address, value bn.Number, note string)                      //根据符号执行代币或基础通证的转账，收据中包含 note 信息（cong）
	TransferBySymbolWithNoteBRC30(orgName string, symbol string, to types.Address, value bn.Number, note string) //根据组织名称及代币符号向指定账户进行指定的BRC30代币的转账,收据中包含note信息
...
type IBlockChainHelper interface {
	...
	RecalcAddressEx(address types.Address, chainName string) types.Address       //根据给定地址和chainName重新计算新地址
	...
	GetTx(txHash string) *std.TxResult         // 根据交易哈希获取交易信息
	GetChainID(address types.Address) string   // 从地址中获取链ID
	GetMainChainID() string                    // 从当前链ID中获取主链ID
	GetChainName(address types.Address) string // 从地址中提取chainName
	GetLocalChainName() string                 // 获取本地chainName
...
type ITokenHelper interface {
	...
	TokenOfNameBRC30(orgName, name string) IToken                                                        //根据组织名称及代币名称获取指定BRC30代币对象
	TokenOfSymbol(symbol string) IToken                                                                  //根据代币符号获取代币或基础通证的信息
	TokenOfSymbolBRC30(orgName, symbol string) IToken                                                    //根据组织名称及代币符号获取指定BRC30代币对象
...
type IIBCHelper interface {
	IbcHash(toChainID string) types.Hash // 计算跨链事务Hash
	Run(f func()) IIBCHelper             // 执行产生需要跨链的收据的代码段

	Register(toChainID string)  // 注册跨链事务
	Notify(toChainIDs []string) // 发起跨链通知
	Broadcast()                 // 发起跨链广播

	CalcBlockHash(h interface{}) types.Hash                                                          // 根据区块头计算区块Hash
	CalcQueueHash(packet ibc.Packet, lastQueueHash types.Hash) types.Hash                            // 根据消息数据及上一个队列Hash计算当前队列Hash
	VerifyPrecommit(pubKey types.PubKey, precommit ibc.Precommit, chainID string, height int64) bool // 校验区块证明数据
}

// IIBCStubHelper the interface for IBCStubHelper
type IIBCStubHelper interface {
	Recast(ibcHash types.HexBytes, orgID, contractName string, receipts []types.KVPair) (bool, []types.KVPair, types.Error)    // 资产重铸
	Confirm(ibcHash types.HexBytes, orgID, contractName string, receipts []types.KVPair) ([]types.KVPair, types.Error)         // 跨链事务确认
	Cancel(ibcHash types.HexBytes, orgID, contractName string, receipts []types.KVPair) ([]types.KVPair, types.Error)          // 跨链事务回滚
	TryRecast(ibcHash types.HexBytes, orgID, contractName string, receipts []types.KVPair) (bool, []types.KVPair, types.Error) // 资产预重铸
	ConfirmRecast(ibcHash types.HexBytes, orgID, contractName string, receipts []types.KVPair) ([]types.KVPair, types.Error)   // 跨链事务中转确认
	CancelRecast(ibcHash types.HexBytes, orgID, contractName string, receipts []types.KVPair) ([]types.KVPair, types.Error)    // 跨链事务中转取消
	Notify(ibcHash types.HexBytes, orgID, contractName string, receipts []types.KVPair) ([]types.KVPair, types.Error)          // 跨链通知
}
