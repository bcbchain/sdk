...
//
// 2018-12-7, notes:
// Now, we implemented encoder to support signed integers and map,
// boolean values is supported already.
//
// Boolean value encodes as 0x01 for true or 0x80 for false
//
// An signed integer value is converted to unsigned integer correspondingly
// and encoded as unsigned integer value.
//
// Map values are encoded as an Key-Value slices of all their encoded
// public fields. Recursive map types are supported.
...
func makeWriter(typ reflect.Type, ts tags) (writer, error) {
	...
	case typ.AssignableTo(reflect.PtrTo(number)):
		return writeNumberPtr, nil
	case typ.AssignableTo(number):
		return writeNumber, nil
	case isUint(kind):
...
func writeNumberPtr(val reflect.Value, w *encbuf) error {
	ptr := val.Interface().(*bn.Number)
	if ptr == nil {
		w.str = append(w.str, 0)
		return nil
	}
	w.encodeString(ptr.Bytes())
	return nil
}

func writeNumber(val reflect.Value, w *encbuf) error {
...
func writeMap(val reflect.Value, w *encbuf) error {
	ks := val.MapKeys()
	if ks == nil {
		return nil
	}
	...
	enKVs := make([][]byte, len(ks))
	for i, k := range ks {
		temp := make([][]byte, 2)
		//encode key
		kb, err := EncodeToBytes(k.Interface())
		if err != nil {
			return err
		}
		temp[0] = kb
		//encode value
		v := val.MapIndex(k)
		vb, err := EncodeToBytes(v.Interface())
		if err != nil {
			return err
		}
		temp[1] = vb
		//encode key & value slice
		bts, err := EncodeToBytes(temp)
		if err != nil {
			return err
		}
		enKVs[i] = bts
