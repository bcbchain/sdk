...
//
// 2018-12-7, notes:
// Now, we implemented decoder to support signed integers and map,
// boolean values is supported already.
//
// To decode into an boolean type, he input must be a string and up to 1 byte.
//
// To decode into an signed integer type, the input must also be an RLP
// string. The bytes are interpreted as a big endian representation of
// the integer. If the RLP string is larger than the bit size of the
// type,
//
// To decode into an map type, the input must also be an RLP string.
// And its key-value pair slices must also be an RLP string.
...
var (
	...
	number           = reflect.TypeOf(bn.Number{})
)
...
func makeDecoder(typ reflect.Type, tags tags) (dec decoder, err error) {
	...
	case typ.AssignableTo(reflect.PtrTo(number)):
		return decodeNumberPtr, nil
	case typ.AssignableTo(number):
		return decodeNumber, nil
	case isUint(kind):
...
func decodeMap(s *Stream, val reflect.Value) error {
	b, err := s.Bytes()
	if err != nil {
		return wrapStreamError(err, val.Type())
	}
	//call make() to allocate memory for map
	val.Set(reflect.MakeMap(val.Type()))

	key := reflect.New(val.Type().Key())
	value := reflect.New(val.Type().Elem())
	var bbt [][]byte
	//decode to []keyvalue slices
	err = DecodeBytes(b, &bbt)
	if err != nil {
		return wrapStreamError(err, val.Type())
	}
	for _, v := range bbt {
		var bbv [][]byte
		//decode to keyvalue slice
		err = DecodeBytes(v, &bbv)
		if err != nil || len(bbv) != 2 {
			return wrapStreamError(err, val.Type())
		}
		//decode key
		err = DecodeBytes(bbv[0], key.Interface())
		if err != nil {
			return wrapStreamError(err, val.Type())
		}
		if value.Elem().Kind() == reflect.Map {
			value.Elem().Set(reflect.MakeMap(value.Elem().Type()))
		}
		err = DecodeBytes(bbv[1], value.Interface())
		if err != nil {
			return wrapStreamError(err, val.Type())
		}
		//set key and value
...
func decodeNumber(s *Stream, val reflect.Value) error {
	return decodeNumberPtr(s, val.Addr())
}

func decodeNumberPtr(s *Stream, val reflect.Value) error {
	b, err := s.Bytes()
	if err != nil {
		return wrapStreamError(err, val.Type())
	}
	i := val.Interface().(*bn.Number)
	if i == nil {
		i = new(bn.Number)
...
func makeStructDecoder(typ reflect.Type) (decoder, error) {
	...
	dec := func(s *Stream, val reflect.Value) (err error) {
		...
		for _, f := range fields {
			vf := val.Field(f.index)
			//If a structure has a map field, malloc and map pointer and set empty value to
			//then decode bytes to map and set back to field.
			if vf.Kind() == reflect.Map {
				mapval := reflect.New(vf.Type())
				mapval.Elem().Set(reflect.MakeMap(vf.Type()))
				err = f.info.decoder(s, mapval.Elem())
				if err == nil {
					vf.Set(mapval.Elem())
				}
			} else {
