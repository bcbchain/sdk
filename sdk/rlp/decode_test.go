...
var (
	veryBnNumber = bn.N(0xFFFFFFFFFFFFFF).Lsh(16).Add(bn.N(0xFFFF))
	bn16         = bn.N(16)
	bn_65535     = bn.N(-65535)
)
...
var decodeTests = []decodeTest{
	...
	//signed integers
	{input: "81ff", ptr: new(int8), value: int8(-1)},
	{input: "82ffff", ptr: new(int16), value: int16(-1)},
	{input: "84ffffffff", ptr: new(int32), value: int32(-1)},
	{input: "88ffffffffffffffff", ptr: new(int64), value: int64(-1)},
	{input: "88ffffffffffffffff", ptr: new(int), value: int(-1)},

	{input: "8181", ptr: new(int8), value: int8(-127)},
	{input: "82ff81", ptr: new(int16), value: int16(-127)},
	{input: "84ffffff81", ptr: new(int32), value: int32(-127)},
	{input: "88ffffffffffffff81", ptr: new(int64), value: int64(-127)},
	{input: "88ffffffffffffff81", ptr: new(int), value: int(-127)},
	...
	// bn numbers
	//	{input: "00", ptr: new(bn.Number), value: bn.N(0)},
	{input: "820001", ptr: new(bn.Number), value: bn.N(1)},
	{input: "820010", ptr: new(bn.Number), value: bn.N(16)},
	{input: "8200C0", ptr: new(bn.Number), value: bn.N(192)},
	{input: "8400FFFFFF", ptr: new(bn.Number), value: bn.N(0xFFFFFF)},
	{input: "8500FFFFFFFF", ptr: new(bn.Number), value: bn.N(0xFFFFFFFF)},
	{input: "8600FFFFFFFFFF", ptr: new(bn.Number), value: bn.N(0xFFFFFFFFFF)},
	{input: "8700FFFFFFFFFFFF", ptr: new(bn.Number), value: bn.N(0xFFFFFFFFFFFF)},
	{input: "8800FFFFFFFFFFFFFF", ptr: new(bn.Number), value: bn.N(0xFFFFFFFFFFFFFF)},
	{input: "8a00FFFFFFFFFFFFFFFFFF", ptr: new(bn.Number), value: veryBnNumber},

	{input: "01", ptr: new(bn.Number), value: bn.N(1)},
	{input: "10", ptr: new(bn.Number), value: bn.N(16)},
	{input: "81C0", ptr: new(bn.Number), value: bn.N(192)},
	{input: "83FFFFFF", ptr: new(bn.Number), value: bn.N(-65535)},
	{input: "84FFFFFFFF", ptr: new(bn.Number), value: bn.N(-16777215)},
	{input: "85FFFFFFFFFF", ptr: new(bn.Number), value: bn.N(-4294967295)},
	{input: "86FFFFFFFFFFFF", ptr: new(bn.Number), value: bn.N(-1099511627775)},
	{input: "87FFFFFFFFFFFFFF", ptr: new(bn.Number), value: bn.N(-281474976710655)},

	{input: "820010", ptr: new(*bn.Number), value: &bn16},
	{input: "83FF0400", ptr: new(*bn.Number), value: &bn_1024},
	{input: "83FFFFFF", ptr: new(*bn.Number), value: &bn_65535},
	...
	// struct contains map
	{
		input: "CB0189C887C6318483616263",
		ptr:   new(mapstruct),
		value: mapstruct{
			A: 1,
			B: map[string]string{"1": "abc"},
		},
	},
	// RawValue
...
var decodeMapTests = []decodeMapTest{
	//Map
	{input: "81C0", ptr: make(map[string]string), value: map[string]string{}},
	{input: "85C483C23131", ptr: make(map[string]string), value: map[string]string{"1": "1"}},
	{input: "A1E086C531838231308CCB84833535358584333030308BCA84836162638483323030", ptr: make(map[string]string), value: map[string]string{"1": "10", "abc": "200", "555": "3000"}},
	{input: "81C0", ptr: make(map[int]string), value: map[int]string{}},
	{input: "8BCA89C86486C50383666F6F", ptr: make(map[int]simplestruct), value: map[int]simplestruct{100: {A: 3, B: "foo"}}},
	{input: "88C786C56483C205C0", ptr: make(map[int]recstruct), value: map[int]recstruct{100: {5, nil}}},
	{input: "8CCB8AC96487C605C404C203C0", ptr: make(map[int]recstruct), value: map[int]recstruct{100: {5, &recstruct{4, &recstruct{3, nil}}}}},
	{input: "89C887C66484C3010203", ptr: make(map[int]tailRaw), value: map[int]tailRaw{100: {A: 1, Tail: []RawValue{unhex("02"), unhex("03")}}}},
}

func TestDecodeMap(t *testing.T) {
	runMapTests(t, func(input []byte, into interface{}) error {
		return Decode(bytes.NewReader(input), into)
	})
}

func runMapTests(t *testing.T, decode func([]byte, interface{}) error) {
	for i, test := range decodeMapTests {
		input, err := hex.DecodeString(test.input)
		if err != nil {
			t.Errorf("test %d: invalid hex input %q", i, test.input)
			continue
		}
		obj := reflect.New(reflect.TypeOf(test.ptr))
		obj.Elem().Set(reflect.ValueOf(test.ptr))
		err = decode(input, obj.Interface())
		if err != nil && test.error == "" {
			t.Errorf("test %d: unexpected Decode error: %v\ndecoding into %T\ninput %q",
				i, err, obj, test.input)
			continue
		}
		if test.error != "" && fmt.Sprint(err) != test.error {
			t.Errorf("test %d: Decode error mismatch\ngot  %v\nwant %v\ndecoding into %T\ninput %q",
				i, err, test.error, obj, test.input)
			continue
		}
		deref := obj.Elem().Interface()
		if err == nil && !reflect.DeepEqual(deref, test.value) {
			t.Errorf("test %d: value mismatch\ngot  %#v\nwant %#v\ndecoding into %T\ninput %q",
				i, deref, test.value, obj, test.input)
		}
	}
}
