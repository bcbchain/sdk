...
func initGenesis(g *genesis) (smcError types.Error) {
	...
	for _, c := range g.AppStateJSON.GnsContracts {
		...
		cnt.ContractAddrList = make([]types.Address, 0)
		cnt.EffectHeights = make([]int64, 0)
		cnt.Name = c.Name
		cnt.ContractAddrList = append(cnt.ContractAddrList, c.Address)
		cnt.EffectHeights = append(cnt.EffectHeights, c.EffectHeight)
