...
func (th *TokenHelper) RegisterToken(
	...
	sdk.Require(transferMethodExist == true && transferInterExist == true,
	...
	// set token's data
	keyOfToken := std.KeyOfToken(token.Address())
	th.smc.(*sdkimpl.SmartContract).LlState().McSet(keyOfToken, token.(*object.Token).StdToken())

	// set token's name with address's value
	keyOfTokenWithName := std.KeyOfTokenWithName(name)
	th.smc.(*sdkimpl.SmartContract).LlState().McSet(keyOfTokenWithName, &tokenAddr)

	// set token's symbol with address's value
	keyOfTokenWithSymbol := std.KeyOfTokenWithSymbol(symbol)
	th.smc.(*sdkimpl.SmartContract).LlState().McSet(keyOfTokenWithSymbol, &tokenAddr)
