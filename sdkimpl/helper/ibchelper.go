...
// CalcBlockHash cal
func (ih *IBCHelper) CalcBlockHash(h interface{}) types.Hash {
	sdk.Require(h != nil,
		types.ErrInvalidParameter, "cannot calculate nil block header")

	switch reflect.TypeOf(h).String() {

	case "*ibc.Header":
		header, ok := h.(*ibc.Header)
		sdk.Require(ok,
			types.ErrInvalidParameter, "must be sdk ibc Header")

		return ih.calcHashOfHeader(header)

	case "*ibc.Header_2_2":
		header, ok := h.(*ibc.Header_2_2)
		sdk.Require(ok,
			types.ErrInvalidParameter, "must be sdk ibc Header_2_2")

		return ih.calcHashOfHeader_2_2(header)

	default:
		sdk.Require(false,
			types.ErrInvalidParameter, "invalid header type")
...
func (ih *IBCHelper) calcHashOfHeader(h *ibc.Header) types.Hash {
...
func (ih *IBCHelper) calcHashOfHeader_2_2(h *ibc.Header_2_2) types.Hash {
	// 将string装换为time.Time类型，否则计算的blockHash不正确
	t, err := time.Parse(ih.layoutByTime(h.Time), h.Time)
	sdk.RequireNotError(err, types.ErrInvalidParameter)
	aminoHasher := common.AminoHasher
