...
// TokenOfNameBRC30 get BRC30token with orgName and name
func (th *TokenHelper) TokenOfNameBRC30(orgName, name string) sdk.IToken {
	tokenAddr := th.tokenAddressOfOrgNameAndName(orgName, name)
	if tokenAddr == "" {
		return nil
	}
...
// TokenOfSymbolBRC30 get BRC30token with orgName and symbol
func (th *TokenHelper) TokenOfSymbolBRC30(orgName, symbol string) sdk.IToken {
	tokenAddr := th.tokenAddressOfOrgNameAndSymbol(orgName, symbol)
	if tokenAddr == "" {
		return nil
	}
...
func (th *TokenHelper) CheckActivate(address types.Address) error {
	...
	if toChainID != th.smc.Helper().BlockChainHelper().GetMainChainID() {
		...
		if index == len(supportSideChains) || supportSideChains[index] != toChainID {
			return errors.New("side chain never activate current token")
		}
	}
...
// TokenAddressOfName get BRC30token address with name and orgName
func (th *TokenHelper) tokenAddressOfOrgNameAndName(orgName, name string) types.Address {
	key := std.KeyOfBRC30TokenWithName(orgName, name)

	return *(th.smc.(*sdkimpl.SmartContract).LlState().McGetEx(key, new(types.Address)).(*types.Address))
}

// TokenAddressOfName get BRC30token address with name and orgName
func (th *TokenHelper) tokenAddressOfOrgNameAndSymbol(orgName, symbol string) types.Address {
	key := std.KeyOfBRC30TokenWithSymbol(orgName, symbol)
